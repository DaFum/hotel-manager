...
describe("physical operational situations", () => {
  ...
  it.each(["facility.cause.stock", "facility.cause.miseEnPlace"] as const)(
    "evaluates %s constraints as a kitchen cause",
    (cause) => {
      const state = {
        ...base,
        fnb: {
      ...
      expect(described.fnb.kitchen.cause).toBe(cause);
    },
  );
