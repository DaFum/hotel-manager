...
import { compareIds } from "../game/domain/ids";
...
/** Projects the room coordinates generated by the authoritative floor plan. */
export function placeRoomsFromGeometry(
  rooms: readonly LayoutRoom[],
  positionByEntityId: Readonly<
    Record<string, { floor: number; gridX: number; gridY: number }>
  >,
  ...
  return rooms
    .flatMap((room): RoomPlacement[] => {
      const position = positionByEntityId[room.id];
      if (!position) return [];
      const projected = isoProject(
        position.gridX,
        position.gridY,
